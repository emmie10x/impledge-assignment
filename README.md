...
3. **Memoized Recursion**: For each word, a recursive function checks if it can be constructed from smaller words. It tries matching prefixes in the `unordered_set`. To avoid redundant computations and ensure high performance, an array `memo` is used for memoization within each word evaluation.
